        ...
        <?php
            ...
            if(empty($_POST['nom'])){
                ...
                if (!mysqli_connect($host, $user, $userpassword, 'CULM')) {
                    ...
                    $testCode = mysqli_query($link, "select id_examen, code, fk_etudiant from T_EXAMEN where code = '".$_POST['code']."';");
                    $resultCode = mysqli_fetch_array($testCode);
                    //si le code existe
                    if($resultCode['code'] == $_POST['code']){
                        //on vérifie si il n'est attribué à aucun étudiant
                        if($resultCode['fk_etudiant'] == ""){
                            //dans ce cas on met les données dans la bdd
                            $testEtudiant = mysqli_query($link, "select * from T_ETUDIANT where nom = '".$_POST['nom']."' and prenom = '".$_POST['prenom']."' and date_naissance = '".$_POST['naissance']."';");
                            $resultEtudiant = mysqli_fetch_array($testEtudiant);

                            //je passe la premmière lettre du nom et du prenom en majucule
                            $nom = ucfirst($_POST['nom']);
                            $prenom = ucfirst($_POST['prenom']);

                            //si l'étudiant existe déja
                            if($resultEtudiant['nom'] == $nom && $resultEtudiant['prenom'] == $prenom && $resultEtudiant['date_naissance'] == $_POST['naissance']){
                                ...
                                mysqli_query($link, "insert into T_ETUDIANT (nom, prenom, date_naissance) values ('".$nom."', '".$prenom."', '".$_POST['naissance']."');");
                            }

                            if($nombreExamen < 6){
                                //on recupère les valeurs de l'étudiant au cas ou il aurait été créé
                                $testEtudiant2 = mysqli_query($link, "select * from T_ETUDIANT where nom = '".$nom."' and prenom = '".$prenom."' and date_naissance = '".$_POST['naissance']."';");
